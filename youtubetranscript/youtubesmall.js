...
function writeVarint(buf, offset, value) {
  let v = value >>> 0;
  while (v > 0x7f) { buf[offset++] = (v & 0x7f) | 0x80; v >>>= 7; }
  buf[offset] = v;
}
...
class PbReader {
  constructor(bytes) {
    ...
    this.pos = 0; this.len = this.buf.length;
  }
  ...
  uint32() {
    ...
    while (this.pos < this.len) {
      ...
      s += 7; if (s >= 35) { this.pos++; return r >>> 0; }
    }
    return 0;
  }
  bytes() {
...
class PbWriter {
  ...
  bytes(data) {
    ...
    this.uint32(d.length); this._push(d); return this;
  }
  finish() {
...
function pbEncode(obj) {
  ...
  for (const [key, fieldVal] of Object.entries(obj)) {
    ...
    for (const val of items) {
      if (!val) continue;
      if (val.t === 'varint') {
        w.uint32((fn << 3) | 0); w.uint32(val.v);
      } else if (val.t === 'fixed64') {
        w.uint32((fn << 3) | 1); w._push(val.v);
        ...
        w.uint32((fn << 3) | 2); w.bytes(encoded);
      } else if (val.t === 'msg') {
        if (val.dirty) {
          const childBytes = pbEncode(val.v);
          w.uint32((fn << 3) | 2); w.bytes(childBytes);
        } else if (val.raw) {
          w.uint32((fn << 3) | 2); w.bytes(val.raw);
        } else {
          const childBytes = pbEncode(val.v);
          w.uint32((fn << 3) | 2); w.bytes(childBytes);
        }
      } else if (val.t === 'raw') {
        w.uint32((fn << 3) | 2); w.bytes(val.v);
      }
    }
  }
  return w.finish();
...
function tryStr(val) {
  ...
  if (val.t === 'raw') {
    try { return UTF8.decode(val.v); } catch { return null; }
  }
  if (val.t === 'msg' && val.raw) {
    try { return UTF8.decode(val.raw); } catch { return null; }
  }
  return null;
}
...
function fnv1aHash(buffer) {
  ...
  for (let i = 0; i < buffer.length; i++) {
    h ^= buffer[i]; h += (h << 1) + (h << 4) + (h << 7) + (h << 8) + (h << 24); h = h >>> 0;
  }
...
async function asyncPool(concurrency, tasks, taskFn) {
  ...
  async function runner() {
    while (index < tasks.length) { const i = index++; results[i] = await taskFn(tasks[i], i); }
  }
...
async function cacheResponse(cacheKey, body, status, origHeaders, isGzip) {
  ...
  if (status >= 200 && status < 400) {
    try { await caches.default.put(cacheKey, resp.clone()); console.log('✅ 双语字幕写入缓存成功'); }
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      console.log(`Token提取: ${tokenStr ? '成功' : '失败'} | Cache Key: ${cacheKey}`);
